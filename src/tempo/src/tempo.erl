...
-define(EPOCH_ZERO, calendar:datetime_to_gregorian_seconds({{1970, 1, 1}, {0, 0, 0}})).
...
-type format() :: binary() | iso8601 | rfc1123 | rfc2822.
-type datetime_type() :: unix | now | datetime.
-type datetime_value() :: unix_timestamp() | erlang:timestamp() | calendar:datetime().
...
%% @end
-spec parse(format(), binary(), datetime_type()) -> {ok, datetime_value()} |
                                                    {error, format_mismatch}.
parse(Format, Bin, Type) ->
    case Type of
      unix ->
...
%% @end
-spec parse_unix(format(), binary()) -> {ok, unix_timestamp()} | {error, format_mismatch}.
parse_unix(Format, Bin) ->
...
%% @end
-spec format(format(), {datetime_type(), datetime_value()}) -> {ok, binary()} |
                                                               {error, invalid_time} |
                                                               {error, time_overflow}.
...
%%      Datetime will be handled depends on Type.
%% @end
-spec format(format(), datetime_value(), datetime_type()) -> {ok, binary()} |
                                                             {error, invalid_time} |
                                                             {error, time_overflow}.
format(Format, Datetime, Type) ->
    case Type of
      unix ->
          format_unix(Format, Datetime);
      now ->
          format_now(Format, Datetime);
      datetime ->
...
convert_format(X) when is_binary(X) ->
    X;
convert_format(iso8601) ->
    <<"%Y-%m-%dT%H:%M:%SZ">>;
convert_format(rfc1123) ->
    <<"%a, %d %b %Y %H:%M:%S GMT">>;
convert_format(rfc2822) ->
    <<"%a, %d %b %Y %H:%M:%S +0000">>;
convert_format(X) ->
    error(badarg, [X]).
...
%% @end
-spec strptime(binary(), binary()) -> {ok, integer()} | {error, format_mismatch}.
strptime(_Format, _DT) ->
    ?STUB.

%% @private
%% @doc This function will be replaced with NIF's strftime.
%% @end
-spec strftime(binary(), integer()) -> {ok, binary()} | {error, invalid_time}.
strftime(_Format, _DT) ->
    ?STUB.
