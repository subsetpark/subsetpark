...
extra_part() ->
    ...
    ?LET(N, integer(1, 15), vector(N, oneof(ExtraSeqs))).
...
prop_format_parse_now() ->
    ?FORALL({{MegaSecs, Secs, _} = Now, Format},
            {now(), standard_format()},
            try
...
prop_parse_stable_on_binaries() ->
    ?FORALL({Format, Buf}, {binary(), binary()}, valid_parse_return(Format, Buf)).

prop_parse_stable_on_anything() ->
    ?FORALL({Format, Buf}, {any(), any()}, valid_parse_return(Format, Buf)).

prop_format_stable_on_valid_types() ->
    ?FORALL({Format, Time}, {binary(), integer()}, valid_format_return(Format, Time)).

prop_format_stable_on_anything() ->
    ?FORALL({Format, Time}, {any(), any()}, valid_format_return(Format, Time)).
...
proper_test_() ->
    {timeout,
     600,
     ?_assertEqual([], proper:module(tempo_props, [{to_file, user}, {numtests, 5000}]))}.
